            ...
            <div class="list_item nav nav-tabs" role="tablist">
                <a class="item nav-link active" data-toggle="tab" href="#wta_wallet">
                    <div class="number">
                        ...
                        ?>
                        @if(is_float($token_balance) && $token_balance>=1000)
                        <p>{{number_format(explode(".",$token_balance)[0],0,"",",").".".explode(".",$token_balance)[1]}}</p>
                        @else
                        <p>{{number_format($token_balance,0,"",",")}}</p>
                        @endif
                        @if($now->exists() && $token_balance>0)
                        @php
                        $total_price = $now->first()->price*$token_balance;
                        @endphp
                        <p data-usdwta="{{$total_price}}">${{number_format($total_price,2,'.',',')}}</p>
                        @elseif($past->exists() && $token_balance>0)
                        @php
                ...
                <a class="item nav-link">
                    <div class="number">
                        ...
                        //$token_balances = 100.001343;
                        ?>
                        @if(is_float($total_value) && $total_value>=1000)
                        <p>{{number_format(explode(".",$total_value)[0],0,"",",").".".explode(".",$total_value)[1]}}</p>
                        @else
                        <p>{{number_format($total_value,0,"",",")}}</p>
                        @endif
                        @if($now->exists() && $total_value>0)
                        @php
                        $total_price = $now->first()->price*$total_value;
                        @endphp
                        <p data-usdwta="{{$total_price}}">${{number_format($total_price,2,'.',',')}}</p>
                        @elseif($past->exists() && $total_value>0)
                        @php
                ...
                <a class="item nav-link" data-toggle="tab" href="#bnb_wallet">
                    <div class="number">
                        ...
                        <p id="bnb" data-qantity="{{$bnb_balance}}">
                            @php
                            $bnb_bal= explode(".",$bnb_balance);
                            @endphp
                            @if(!empty($bnb_bal[1]) && $bnb_bal[1]=="0000")
                            {{$bnb_bal[0]}}
                            @else
                            {{$bnb_balance}}
                            @endif
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade active show" id="depositwta" role="tabpanel">
                            <div class="input-group mb-0 input-primary">
                                <input class="form-control" id="address_wta" value="{{$bnb_wallet->address}}">
                ...
                <div class="hide bnb_token">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade active show" id="depositbnb" role="tabpanel">
                            <div class="input-group mb-0 input-primary">
                                <input type="text" value="{{$bnb_wallet->address}}" class="form-control" id="address_bnb">
                        ...
                        <div class="tab-pane fade " id="withdrawbnb" role="tabpanel">
                            ...
                            <div class="amount_balance input-group mt-2">
                                <input type="text" data-balance="{{$bnb_balance}}" class="form-control amount_withdraw_bnb" placeholder="Minimum 0.001" />
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @endif
                                    <td>
                                        @if(!empty(json_decode($value->hash_wta)) && $value->hash_wta!='""' && !empty(json_decode($value->hash_wta)->transactionHash))
                                        <a href="https://bscscan.com/tx/{{json_decode($value->hash_wta)->transactionHash}}" target="_blank">
            ...
            <div id="bnb_wallet" class="tab-pane fade" role="tabpanel">
                ...
                <div class="transaction_list">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @if(!empty($transactionsBNB))
                            @foreach($transactionsBNB as $value)
                            <tr>
                                <td>{{$value->created_at}}</td>
                                <td>OUT</td>
                                <td>BNB</td>
                                ...
                                @endif
                                <td>
                                    @if(!empty($value->bnb_hash))
                                    <a href="https://bscscan.com/tx/{{json_decode($value->bnb_hash)->transactionHash}}" target="_blank">
                                        <svg width="9" height="14" viewBox="0 0 9 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M1.61339 0.999999L7.76465 7L1.61339 13" stroke="#EC4C93" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </a>
