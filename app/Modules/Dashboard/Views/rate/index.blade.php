                                ...
                                <table class="table table-sm table-hover mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><a href="/{{Helper_Dashboard::get_patch()}}/{{Helper_Dashboard::get_patch(2)}}/edit/{{$value->id}}" title="chỉnh sửa {{$value->title}}">{{$value->name}}</a></td>
                                            <td>{{$value->bnb}} BNB = {{number_format($value->token,0,"",",")}} {{$settings['ICO_SYMBOL']}}</td>
                                            <td>
                                                @if($value->status)
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right"  x-placement="bottom-start" >
                                                        ...
                                                        @if($value->status==1)
                                                        <a class="dropdown-item text-danger" href="/{{Helper_Dashboard::get_patch(1)}}/{{Helper_Dashboard::get_patch(2)}}/status/{{$value->id}}/0"><i class="fe-lock"></i> Khóa</a>
                                                        @elseif($value->status==0)
                                                        <a class="dropdown-item text-success" href="/{{Helper_Dashboard::get_patch(1)}}/{{Helper_Dashboard::get_patch(2)}}/status/{{$value->id}}/1"><i class="fe-check-circle"></i> Kích hoạt</a>
                                                        @endif
                                                        <div class="dropdown-divider"></div>
                                                        <a class="dropdown-item text-danger" href='/{{Helper_Dashboard::get_patch(1)}}/{{Helper_Dashboard::get_patch(2)}}/delete/[{"id":{{$value->id}}}]'><i class="fe-trash-2"></i> Xóa</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
