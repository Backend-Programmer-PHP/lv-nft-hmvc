...
class TopPlace extends Controller
{
    ...
    {
        $data = TopPlace_Model::select("post.title","post.image","top_place.*","category.category_name")
        ->leftJoin("post","post.id","=","top_place.post_id")
        ->leftJoin("category","category.id","=","post.category_id")
        ->where("top_place.language_id", 1)
        ->whereIn("top_place.status", [0, 1])->orderBy('id', 'desc')->paginate(15);
        $data->setPath('en');
        $row = json_decode(json_encode([
            "title" => "Top Place - En",
        ]));
        return view("Dashboard::topplace.index", compact("row","data"));
    }
    ...
    public function add($lang = "")
    {
        ...
        return view("Dashboard::topplace.add", compact("row","post","settings"));
    }
    ...
    {
        $settings = config('global.settings');
        $data = TopPlace_Model::find($id);
        $post = Post_Model::select("id","title")->where("language_id",$data->language_id)->where("status",1)->get();
        $row = json_decode(json_encode([
            "title" => "Bài viết",
            "desc" => "Chỉnh sửa",
        ]));
        return view("Dashboard::topplace.edit", compact("row", "data", "post" ,"settings"));
    }
    ...
    public function delete($id = "")
    {
        ...
        if (count($list_id) == 1 && isset($list_id[0]->id)) {
            $category = TopPlace_Model::find($list_id[0]->id);
            ...
            foreach ($list_id as $key => $value) {
                $category = TopPlace_Model::find($value->id);
                $category->status = 2; //2 is trash
                $category->save();
            }
    ...
    {
        //echo $status;
        $category = TopPlace_Model::find($id);
    ...
    {
        $data = TopPlace_Model::where("status", 2)->orderBy('id', 'desc')->paginate(15);
        $data->setPath('trash');
        $row = json_decode(json_encode([
            "title" => "Thùng rác - Top Place",
        ]));
        return view("Dashboard::topplace.trash", compact("row", "data"));
    }
    ...
    public function trashDelete($id = "")
    {
        ...
        //xoa mot
        if (count($list_id) == 1) {
            $slide = TopPlace_Model::find($list_id[0]->id);
            if ($slide->delete()) {
